...
public class JobGiver_DisassemblePlayerBuilding : ThinkNode_JobGiver
{
    ...
    protected override Job TryGiveJob(Pawn pawn)
    {
        ...
        {
            List<Thing> allThings = pawn.Map.listerThings.AllThings.Where(x => x.Faction == Faction.OfPlayer && x.def.building != null && x.def != GDFPDefOf.GDFP_QuakkaaiExit && x.def != GDFPDefOf.GDFP_Quakkaai).ToList();
            ...
            if (allThings.Any())
            {
                ...
                if (thing != null)
                {
                    return JobMaker.MakeJob(GDFPDefOf.GDFP_Replicate, thing);
                }
            }
        }
